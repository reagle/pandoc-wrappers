...
logging.basicConfig(
    level=log_level, format="%(levelno)s %(funcName).5s: %(message)s")
...
class MultiReplace(object):
    ...
    def _get_replacement(self, match):  # passed match
        # use match to return replacement
        return self.table_dic.get(match.group(1), "")

    def replace(self, line):
        # pass replacement function
        return self.pattern.sub(self._get_replacement, line)
...
def pandoc_call(md_tmp_file, tex_tmp_file, build_file_base):
    ...
    pandoc_opts = ['-t', 'latex', '--biblatex', '--bibliography=%s' %
                   bib_file, '--no-wrap', '--tab-stop', '8']
...
ACITE_CMD = r'''\\acite[s]?(?:\[[^\]]*\]|\{([^\}]*)\})+'''
CAP_CITEYP_OBJ = re.compile(
    r'''(\\cap\{\})(.*?) ?(%s)''' % ACITE_CMD)  # 110508: added ' ?'
...
def latex_build(
        dst_dir, src_dir, build_file, build_file_base, build_file_name):

    os.chdir(dst_dir)
    if args.commit:
        call(['git', 'ci', '-m', args.commit, src_dir])
    if args.fast:  # run latex once
        call(['pdflatex', '--src-specials',
              '-interaction=nonstopmode', build_file_base])
    else:
        os.chdir(dst_dir)  # bibtex8 only working in cwd
        call(['pdflatex', '--src-specials',
              '-interaction=nonstopmode', build_file_name])
...
def main(args, files):
    ...
    if len(files) > 1:
        ...
        elif path.isfile(fn) is True:  # single source file
    ...
    for ifile in files:
        ...
        md_tmp_file = dst_dir + \
            path.splitext(ifile)[0] + '.md.tmp'  # tweaked markdown
        tex_tmp_file = dst_dir + \
            path.splitext(ifile)[0] + '.tex.tmp'  # pandoc latex
...
if '__main__' == __name__:
    ...
    arg_parser = argparse.ArgumentParser(
        description='Build latex book from markdown files')
    ...
    if args.verbose == 1:
        log_level = logging.CRITICAL  # DEBUG
    elif args.verbose == 2:
